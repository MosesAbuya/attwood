...
$doc -> addStyleSheet(JUri::base(true).'/components/com_tz_portfolio_plus/css/jquery.fancybox.min.css');
$doc -> addScript(JUri::base(true).'/components/com_tz_portfolio_plus/js/jquery.fancybox.min.js');
$doc -> addScript(TZ_Portfolio_PlusUri::base(true).'/templates/maya/js/lightbox.js');
...
if ($paddingCustom) {
    $doc -> addStyleDeclaration('.tplMaya .tp-item-box-container {padding: '.$paddingCustom.';}.tplMaya #portfolio {margin-left: -'.$paddingCustom.';margin-right: -'.$paddingCustom.';}');
}
...
?>
<div id="tpPortfolio" class="<?php echo $bootstrapClass;?>tplMaya<?php echo $this->pageclass_sfx;?>">
    ...
    <div id="tz_options" class="tpFilter clearfix">
        ...
        if($params -> get('tz_show_filter',1)):?>
            <div class="option-combo<?php echo $dropdownStyle?' dropdown':''; ?>">
                <?php if($dropdownStyle){ ?>
                    <button type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" class="btn btn-default btn-outline-secondary btn-sm"><?php
                        echo JText::_('TP_STYLE_MAYA_FILTER'); ?> <span class="tp tp-angle-down option-icon"></span></button>
                <?php } ?>
                <div id="filter" class="option-set clearfix<?php echo $dropdownStyle?' dropdown-menu':''; ?>" data-option-key="filter">
                    <a href="#show-all" data-option-value="*" class="<?php echo $dropdownStyle?'dropdown-item':'tpFilterItem';
        ...
        <?php if($params -> get('show_sort',0) AND $sortfields = $params -> get('sort_fields',array('date','hits','title'))):
            ...
            <div class="option-combo<?php echo $dropdownStyle?' dropdown':'';?>">
                <?php if($dropdownStyle){ ?>
                <button type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" class="btn btn-default btn-outline-secondary btn-sm"><?php
                        echo JText::_('TP_STYLE_MAYA_SORT'); ?> <span class="tp tp-angle-down option-icon"></span></button>
                <?php } ?>

                <div id="sort" class="option-set clearfix<?php echo $dropdownStyle?' dropdown-menu':' hasTooltip';
                ?>" data-option-key="sortBy"<?php echo $dropdownStyle?'':' data-toggle="tooltip" title="'
                    .JText::_('TP_STYLE_MAYA_SORT').'"'; ?>>
                    <?php
                    foreach($sortfields as $sortfield):
                        switch($sortfield):
                            case 'title':
                                ?>
                                <a class="<?php
                                echo $dropdownStyle?'dropdown-item':'tpFilterItem';
                            ...
                            case 'date':
                                ?>
                                <a class="<?php echo $dropdownStyle?'dropdown-item':'tpFilterItem';
        ...
        <?php if($params -> get('show_layout',0)):?>
            <div class="option-combo<?php echo $dropdownStyle?' dropdown':''; ?>">
                <?php if($dropdownStyle){ ?>
                <button type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" class="btn btn-default btn-outline-secondary btn-sm"><?php
                        echo JText::_('TP_STYLE_MAYA_LAYOUT'); ?> <span class="tp tp-angle-down option-icon"></span></button>
                <?php }?>
                <div id="layouts" class="option-set clearfix<?php echo $dropdownStyle?' dropdown-menu':' hasTooltip';
                ?>" data-option-key="layoutMode"<?php echo $dropdownStyle?'':' data-toggle="tooltip" title="'
                    .JText::_('TP_STYLE_MAYA_LAYOUT').'"'; ?>>
                    <?php
