...
class PlgTZ_Portfolio_PlusContentModelGallery extends JModelList
{
...
//            .$db -> quote('"contentid":"').', -1), '.$db -> quote('"').',1)'.')');
        ...
        if($addon = TZ_Portfolio_PlusPluginHelper::getPlugin('content', 'gallery')) {
            $query->where('d.extension_id =' .(int) $addon -> id);
        }
        $query -> where('d.element ='.$db -> quote('gallery'));
